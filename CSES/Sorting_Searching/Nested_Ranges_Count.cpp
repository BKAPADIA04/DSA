    ...
    if(t == 1) {
        ...
        for(ll i = t - 2;i >= 0;i--) {
            ...
            if(right_end >= min_right) {
                auto it = s.upper_bound(right_end);
                contains[v[i][2]] = distance(s.begin(), it);
            }
            s.insert(v[i][1]);
        ...
        for(ll i = 1;i < t;i++) {
            ...
            if(right_end <= max_right) {
                auto it = ss.lower_bound(right_end);
                // cout << right_end << " " << *it << endl;
                if (it != ss.end()) {
                    contained[v[i][2]] = distance(it, ss.end());
                } else {
                    contained[v[i][2]] = 0; 
                }
            }

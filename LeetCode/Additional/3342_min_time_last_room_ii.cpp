...
public:
    ...
    int minTimeToReach(vector<vector<int>>& nums) {
        ...
        while(!pq.empty()) {
            ...
            int time = top[0]; int x = top[1]; int y = top[2]; int state = top[3];
            pq.pop();
            ...
            for(int i = 0;i < 4;i++) {
                ...
                if(isValid(n,m,x_temp,y_temp)) {
                    int temp_time = 0;
                    if(state == 0) {
                        temp_time = max(time + 1, nums[x_temp][y_temp] + 1); 
                    }
                    else {
                        temp_time = max(time + 2, nums[x_temp][y_temp] + 2); 
                    }
                    if(temp_time < dist[x_temp][y_temp]) {
                        dist[x_temp][y_temp] = temp_time;
                        pq.push({temp_time,x_temp,y_temp,1 - state});
                    }
                }
            }
        }

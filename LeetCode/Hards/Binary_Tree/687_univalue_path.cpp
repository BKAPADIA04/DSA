...
public:
    ...
    pair<int,int>check(TreeNode * root, int &answer) {
        if(root == nullptr) {
            return {0,-1001}; // length, value of node
        }

        pair<int,int>lh = check(root->left,answer);
        pair<int,int>rh = check(root->right,answer);

        int tempL = (root->val == lh.second ? lh.first : 0);
        int tempR = (root->val == rh.second ? rh.first : 0);

        answer = max(answer, 1 + tempL + tempR);

        return {1 + max(tempL,tempR),root->val};
    }

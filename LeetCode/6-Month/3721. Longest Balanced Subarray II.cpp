class SegmentTree {
    public:
    int n;
    vector<int>sum,mini,maxi;

    SegmentTree(int n) {
        this->n = n;
        sum = vector<int>(4*n,0);
        mini = vector<int>(4*n,0);
        maxi = vector<int>(4*n,0);
    }
    
    void pull(int node) {
        int lc = node * 2, rc = node * 2 + 1;
        sum[node] = sum[lc] + sum[rc];
        mini[node] = min(mini[lc], sum[lc] + mini[rc]);
        maxi[node] = max(maxi[lc], sum[lc] + maxi[rc]);
    }
    ...
    bool exist(int node, int sum_before, int target) const {
        ...
        return mini[node] <= need && need <= maxi[node];
    }
